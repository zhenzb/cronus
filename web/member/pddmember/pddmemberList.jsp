...
<head>
    ...
    <script>
        ...
        layui.use(['element','laydate','table'], function(){
            ...
            table.render({
                elem: '#test'
                ,url:'${pageContext.request.contextPath}/member?method=getMemberListPage&member_level='+id
                //,width: 1900
                ,height: 580
                ,cols: [[
                    ...
                    ,{field:'member_level', width:111, title: '会员等级', templet:'#member_levelTpl',align:'center'}
                    ...
                    ,{field:'registration_time', width:200, title: '注册时间',templet: '#create_timeTpl',align:'center'}
                    /*,{field:'source', width:95, title: '注册来源',templet:'#sourceTpl',align:'center'}*/
                    ,{field:'registration_time', width:200, title: '等级变更时间',templet:'#create_timeTpl',align:'center'}
                    ,{field:'pare_num', width:119, title: '上级数量',align:'center'}
                    ,{field:'self_num', width:119, title: '拉粉数量',align:'center'}
                    /*,{field:'countOrder', width:101, title: '成交订单数',align:'center'}*/
                    ,{field:'countOrder', width:300, title: '备注',align:'center'}
                    ,{field:'wealth', width:230, title: '操作',align:'center',toolbar:"#barDemo",fixed: 'right'}
                ]]
                ,limit:20
...
//                                alert(JSON.stringify(array));
        ...
        /*20{{ d.registration_time.substr(0,2) }}-{{ d.registration_time.substr(2,2) }}-{{ d.registration_time.substr(4,2) }} {{ d.registration_time.substr(6,2) }}:{{ d.registration_time.substr(8,2) }}:{{ d.registration_time.substr(10,2) }}</span>*/
            document.getElementById("nameid").innerHTML=(nick_name);
            var str = "20"+registration_time;
            document.getElementById("registerTimeId").innerHTML=(str.substr(0,4)+"-"+str.substr(4,2)+"-"+str.substr(6,2)+" "+str.substr(8,2)+":"+str.substr(10,2)+":"+str.substr(12,2));
            console.log("等级"+document.getElementById("ss"));
            // console.log(registration_time)
            document.getElementById("ids").innerHTML=(id);
            document.getElementById("ss").innerHTML=(member_level == 1?"普通会员":member_level == 2?"小掌柜":"大掌柜");
            variable = id;
            //获取订单中获取订单状态
            var from = layui.from;
            $.ajax({
    ...
    <script type="text/html" id="barDemo">
        <input id="switch{{d.id}}" type="checkbox" name="status{{d.id}}" value="{{d.status}}" lay-skin="switch"
               lay-text="启用|<span style='color:#FF0000; '>禁用</span>"
               data-value="{{d.id}}"
               lay-filter="statusFilter" {{ d.status==1 ? 'checked' : '' }} >
        &nbsp&nbsp&nbsp&nbsp&nbsp
        <a class="layui-btn" lay-event="detail" onclick="look_orders({{d.id}},{{d.nick_name}},{{d.registration_time}},{{d.member_level}})"  style="height: 24px;line-height: 25px;width:59px;background-color:#1E9FFF;text-align:center;font-size: 12px;color: #FFFFFF;border-radius:20px;border:none" >查看</a>
...
<script type="text/html" id="barDemo1">
    ...
    {{#  } else { }}
    <a class="layui-btn layui-btn-xs" onclick="disableaa({{d.id}})"><span style="color: red">禁用</span></a>
    {{#  } }}
...
<div class="layui-body">
    ...
    <div id="listOrdersDiv" style="display: none;padding: 15px;">
        <div style="text-align: center;">
            <span style="font-size:18px">用户名：</span><span id="nameid"></span>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
            <span style="font-size:18px">ID:</span><span id="ids"></span>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
            <span style="font-size:18px">注册时间：</span><span id="registerTimeId"></span>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
            <span style="font-size:18px">会员等级:</span> <span id="ss"></span>
            <div style="margin-left: 307px;margin-top: 20px;">
                ...
                <a class="layui-btn" lay-event="edit"   onclick="look_member_top(variable)"  style="height: 20px;line-height: 20px;width:100px;background-color:#1E9FFF;text-align:center;font-size: 12px;color: #FFFFFF;border-radius:20px;border:none">查看上级会员</a>
                <a class="layui-btn"  onclick="look_member_bo(variable)"  style="height: 20px;line-height: 20px;width:100px;background-color:#1E9FFF;text-align:center;font-size: 12px;color: #FFFFFF;border-radius:20px;border:none">查看下级会员</a>
            </div>
        </div>
        <br>
