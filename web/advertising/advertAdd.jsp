...
<%
    ...
    String max_end_time = request.getParameter("max_end_time");
%>

<script>

    var page_location,position,size,next_advertNum,advert_url_id,imgId,linkName;
    ...
    layui.use(['upload','laydate', 'element', 'form'], function () {
        ...
        $.ajax({
            ...
            url: "${ctx}/advertising",
            data: "method=getInPosition&id="+position_id,
            dataType: "json",
        ...
        $('#submit').on('click', function (){
            ...
            if ($("#advert_name").val() == "") {
                ...
                $.ajax({
                    ...
                    cache :true,// 不使用ajax缓存
                    contentType : "application/json",
                    url: "${ctx}/advertising?method=addAdvert",
                    data: {
...
//                                window.location.reload();
        ...
        $(document).ready(function() {
            var max_Time = Number(<%=max_end_time%>) + 1;
            var toStr = max_Time.toString();
            var next_start_time = "20" + toStr.substr(0, 2) + "-" + toStr.substr(2, 2) + "-" + toStr.substr(4, 2)
                + " " + toStr.substr(6, 2) + ":" + toStr.substr(8, 2) + ":" + toStr.substr(10, 2);
            $("#start_time").val(next_start_time);
        });
...
//                                $("#advert_url_id").val(advert_url_id);
            ...
            <%--<div style="padding:20px 5px 0px 50px">--%>
                <%--广告链接ID：--%>
                <%--<input style="width: 50px;height:30px;margin-left: 10px" type="text" id="advert_url_id" value=" " readonly="readonly"/>--%>
            <%--</div>--%>
            <div style="padding:20px 5px 0px 50px">
                广告链接名称：
